...
def random_clicks(mask, class_id = 1, prompts_number=10):
    # return input_point = np.array([[x1, y1], [x2, y2]]) input_label = np.array([1/0, 1/0])
    indices = np.argwhere(mask == class_id) # the indices is (y x)
    indices[:, [0,1]] = indices[:, [1,0]] # the prompt should be (x y)
    point_label = 1
    if len(indices) == 0:
        point_label = 0
        indices = np.argwhere(mask != class_id)
...
def pos_neg_clicks(mask, class_id=1, pos_prompt_number=1, neg_prompt_number=1):
    ...
    #pos_prompt = pos_indices[len(pos_indices)//2]
    pos_label = np.repeat(pos_label, pos_prompt_number)
...
class ImageToImage2D(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.prompt == 'click':
            ...
            #mask_point = cv2.resize(mask,(self.img_size, self.img_size),interpolation=cv2.INTER_NEAREST)
            if 'train' in self.split:
                # pt, point_label = random_click(np.array(mask), class_id)
                pt, point_label = pos_neg_clicks(np.array(mask), class_id)
                ...
                bbox = fixed_bbox(np.array(mask), class_id, self.img_size)
                #pt = random_click(np.array(mask_point), class_id)
