...
def _build_samct(
    ...
    if checkpoint is not None:
        ...
        try:
            ...
            new_state_dict = load_from2(samct, state_dict, image_size, patch_size)
            samct.load_state_dict(new_state_dict)
    return samct

def load_from(samct_w_prompt, samct_dict, image_size, patch_size): # load the positional embedding
    samct_w_prompt_dict = samct_w_prompt.state_dict()
    new_samct_dict = {}
    for k,v in samct_dict.items():
        if k[:7] == 'module.':
            new_samct_dict[k[7:]] = v
        else:
            new_samct_dict[k] = v
    dict_trained = {k: v for k, v in new_samct_dict.items() if k in samct_w_prompt_dict}
    samct_w_prompt_dict.update(dict_trained)
    return samct_w_prompt_dict

def load_from2(samct, sam_dict, image_size, patch_size): # load the positional embedding
